...
- PUSH通知基盤開発の管理画面及び、通知機能の設計・開発（主としてフロントエンド担当だが、AWS/Go なども対応。また、スクラム開発のDEV兼SMとしてチーム開発運用にコミット）
- DMMのトップページの開発・保守運用（スクラム開発のDEVとして参画）
- [現] 横断部署（CTO室）にて会社全体のフロントエンドにおける評価制度・採用（新卒・中途）・事業支援（設計相談・ソースコードレビューなど）を担当

### 2015.10 - インターリンク株式会社
- オウンドメディア・サービス（会員300万規模）の大規模リプレイス・運用（自社CMSの全体設計、管理画面・コンシューマー向けUIのCSS及びコンポーネント設計・実装担当）
- 社内教育（フロントエンド）担当
- CDN設置作業
